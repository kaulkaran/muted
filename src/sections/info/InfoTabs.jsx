...
const InfoTabs = () => {
  // ✅ get media list from redux
  const mediaCount = useSelector((state) => state.media.items.length);
  ...
  return (
    <div className="flex items-center gap-6 border-b border-black/5 pb-1 mb-6">
      <button className="text-sm font-bold text-[#141415] dark:text-white border-b-2 border-[rgb(var(--primary))] pb-2">
        Media
        <span className="ml-1 text-[10px] bg-[rgb(var(--primary))] text-white px-1.5 py-0.5 rounded-full">
          {mediaCount}
        </span>
      </button>
